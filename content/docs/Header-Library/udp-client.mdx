...
int main()
{
    ...
    client.onMessageReceived = [&](const size_t size, const FUdpMessage message) {
...
* Event called when a connection is estabilished
...
* Event called when a connection is closed
...
* Event called when every time It's try recconnect
...
* Size_t param is the size of message and FUdpMessage is the message struct
...
* Event called if any error occur during async process
