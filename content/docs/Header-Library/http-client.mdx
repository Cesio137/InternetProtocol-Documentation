...
* Event called when payload process has been finished
...
* Event called when request is done
...
* Event called when request is canceled
...
* First `int` param get the bytes sent and second `int` get the bytes received from request
...
* `int` param get the number of attemps
...
* `int` param is error code and `std::string` param is error message
